...
function StudentNavLink({ to, end, label, icon: Icon, onNavigate }) {
  ...
  return (
    <NavLink
      ...
      onClick={handleClick}
      data-testid={`student-nav-${to.replace("/student/", "").replace(/\//g, "-")}`}
      className={({ isActive }) =>
        [
...
function getStudentInitial(profile) {
  const name = getStudentDisplayName(profile);
  return name.charAt(0) || "د";
}
    ...
    <>
      ...
      <nav ref={navRef} className="relative flex flex-1 flex-col gap-1 px-3">
        ...
        {navItems.map((item, index) => (
          ...
          >
            <StudentNavLink {...item} onNavigate={closeSidebar} />
          </div>
        ))}
      </nav>
      ...
      <main className="flex min-h-screen min-w-0 flex-1 flex-col">
        <header className="flex h-[60px] shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 transition-colors duration-200 sm:px-6 dark:border-slate-700 dark:bg-[#0f172a]">
          ...
          <button
            ...
            data-testid="student-dark-mode-toggle"
